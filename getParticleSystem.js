...
function getParticleSystem(params) {
  const { camera, parent, rate, texture } = params;
  const uniforms = {
    diffuseTexture: {
  ...
  function _UpdateParticles(
    ...
    for (let p of _particles) {
      ...
      p.position.z += Math.pow(frequency, pulsationDegree) * 0.000006 * speed; // * Динамические величины
  ...
  function update(timeElapsed, frequency, color, speed, pulsationDegree) {
    _AddParticles(timeElapsed);
    _UpdateParticles(timeElapsed, frequency, color, speed, pulsationDegree);
    _UpdateGeometry();
  }
  return { update };
}
